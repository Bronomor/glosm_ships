...
#include "3dsModel/model3DS.h"
#include "3dsModel/Vector2.h"
#include <GL/glut.h>
...
{
	if (m_specialTransform == number) return; // ju¿ jest  
...
// - int: id tekstury: jeli wiêksze od 0 to wymusa automatyczne teksturowanie t¹ tekstur¹, jeli 0 lub brak - przyjmuje teksturowanie z pliku
// - bool: centrowanie obiektu: jeli true lub brak: za pozycjê obiektu przymuje jego geometryczny rodek, jeli false - przyjmje wspó³rzêdne z pliku
void model3DS::draw(int TextureID, bool centruj){
	...
	if (TextureID){ //gdy podano niezerow¹ teksturê nale¿y w³aczyæ automatyczne generowanie tekstur, w przeciwnym wypadku - koordynaty tekstury bazowej z modelu
	...
	if (texGenS == GL_TRUE) glEnable(GL_TEXTURE_GEN_S); else glDisable(GL_TEXTURE_GEN_S); // ustawienie poprzednich wartoci
	if (texGenS == GL_TRUE) glEnable(GL_TEXTURE_GEN_T); else glDisable(GL_TEXTURE_GEN_T);
	m_stereo = false;
}

// ustala funkcjê specjaln¹ (programowy vertex shader)
