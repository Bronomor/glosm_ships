...
public:
	...
	void setDiffuseColor(const float* newColor){std::memcpy(m_diffuseColor,newColor,sizeof(float)*3);}
	void setAmbientColor(const float* newColor){std::memcpy(m_ambientColor,newColor,sizeof(float)*3);}
	void setSpecularColor(const float* newColor){std::memcpy(m_specularColor,newColor,sizeof(float)*3);}
...
private:
	...
	int m_compiledListRight;


public:

	mesh3DS(model3DS *parentModel):m_parentModel(parentModel){ m_specialTransform = 0;}
	
    void buildMesh();
	void calculateNormals();
	void sortFacesByMaterial();
	void draw(int textureID = 0, bool pregenetateList = false);
	void setSpecialTransform(int number); 
	double calculateSpecialTransformX(int vertexIndex);
	double calculateSpecialTransformY(int vertexIndex);
	double calculateSpecialTransformZ(int vertexIndex);
	...
	int getNumFaces()const{return (int)m_faces.size();}
  	const int getListLeft()const{return m_compiledListLeft;}
	const int getListRight()const{return m_compiledListRight;}
	...
	void setListLeft(const int listID){m_compiledListLeft = listID;}
	void setListRight(const int listID){m_compiledListRight = listID;}
...
private:
	...
	bool readChunk(std::ifstream *modelFile, const int objectStart, const int objectLength);
